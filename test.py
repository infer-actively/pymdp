...
if __name__ == "__main__":
    ...
    print('SPM Dot Test, Case 1a Test 1: {}'.format(np.isclose(result1_matlab,result1_python).all()==True))

    result2_python = A.dot(s_array, return_numpy = True)
    print('SPM Dot Test, Case 1a Test 2: {}'.format(np.isclose(result2_matlab,result2_python.astype(float)[:,np.newaxis]).all()==True))

    result3_python = A.dot(s_array, dims_to_omit = [1], return_numpy = True)
    print('SPM Dot Test, Case 1a Test 3: {}'.format(np.isclose(result3_matlab,result3_python).all()==True))
    ...
    result1_python = A.dot(o_array, return_numpy = True)
    print('SPM Dot Test, Case 1b Test 1: {}'.format(np.isclose(result1_matlab,result1_python).all()==True))

    result2_python = A.dot(s_array, return_numpy = True)
    print('SPM Dot Test, Case 1b Test 2: {}'.format(np.isclose(result2_matlab,result2_python.astype(float)[:,np.newaxis]).all()==True))

    result3_python = A.dot(s_array, dims_to_omit = [1], return_numpy = True)
    print('SPM Dot Test, Case 1b Test 3: {}'.format(np.isclose(result3_matlab,result3_python).all()==True))
