"""
Unit tests to compare outputs of different sequence-based (VMP) state inference methods.
...
All optimized VMP methods assume trivial B_dependencies (each factor depends
...
# Import VMP setup helpers (sequence-based inference with trivial B_dependencies)
...
from pymdp.algos import (
    run_vmp_hybrid,
    run_vmp_end2end_padded,
    run_vmp_clustered_end2end
)

# Import original inference method
from pymdp.inference import update_posterior_states

class TestInferStatesVmpComparison(unittest.TestCase):
    """Test that all VMP inference methods produce identical results."""
    ...
    def _test_single_spec_with_batch(self, spec, batch_size=4, A_sparsity_level=None, use_sparsity=False):
        ...
        # The optimized VMP routines assume trivial B_dependencies
        ...
        # The optimized VMP methods operate on dense log-likelihoods, so the
        ...
        # === Original VMP method (from pymdp.inference) ===
        ...
        infer_states_vmp_hybrid_jit = jit(partial(infer_states_vmp_hybrid, A_shapes=A_shapes, num_states=num_states, A_dependencies=A_dependencies, B_dependencies=B_dependencies, num_iter=self.NUM_ITER, tau=self.TAU))
        qs_hybrid = infer_states_vmp_hybrid_jit(obs_padded_hybrid, A_padded_hybrid, D, past_actions, B_padded)
        ...
        infer_states_vmp_clustered_hybrid_jit = jit(partial(infer_states_vmp_clustered_hybrid, c2o_mapping=c2o_mapping, A_shapes=A_shapes, num_states=num_states, A_dependencies=A_dependencies, B_dependencies=B_dependencies, num_iter=self.NUM_ITER, tau=self.TAU))
        qs_clustered_hybrid = infer_states_vmp_clustered_hybrid_jit(obs_clusters_hybrid, A_clusters_hybrid, D, past_actions, B_padded)
        ...
        infer_states_vmp_hybrid_block_jit = jit(partial(infer_states_vmp_hybrid_block, state_shapes=state_shapes, cuts=cuts, num_states=num_states, A_dependencies=A_dependencies, B_dependencies=B_dependencies, num_iter=self.NUM_ITER, tau=self.TAU, use_einsum=False))
        qs_hybrid_block = infer_states_vmp_hybrid_block_jit(A_big, obs_concat, D, past_actions, B_padded)
        ...
        infer_states_vmp_clustered_hybrid_block_jit = jit(partial(infer_states_vmp_clustered_hybrid_block, shape_groups=state_shapes_block, cut_groups=cuts_block, group_mapping=group_mapping, num_states=num_states, A_dependencies=A_dependencies, B_dependencies=B_dependencies, num_iter=self.NUM_ITER, tau=self.TAU))
        qs_clustered_hybrid_block = infer_states_vmp_clustered_hybrid_block_jit(A_block, obs_block, D, past_actions, B_padded)
        ...
        infer_states_vmp_e2e_jit = jit(partial(infer_states_vmp_end2end_padded, num_states=num_states, A_dependencies=A_dependencies, B_dependencies=B_dependencies, num_iter=self.NUM_ITER, tau=self.TAU, sparsity=sparsity))
        qs_end2end = infer_states_vmp_e2e_jit(obs_padded_e2e, A_padded_e2e, D, past_actions, B_padded)
        ...
        infer_states_vmp_clustered_e2e_jit = jit(partial(infer_states_vmp_clustered_end2end, c2o_mapping=c2o_mapping, c2s_mapping=c2s_mapping, max_state_dims=max_state_dims, num_states=num_states, A_dependencies=A_dependencies, B_dependencies=B_dependencies, num_iter=self.NUM_ITER, tau=self.TAU, sparsity=sparsity))
        qs_clustered_end2end = infer_states_vmp_clustered_e2e_jit(obs_clusters_e2e, A_clusters_e2e, D, past_actions, B_padded)

        # Compare all methods
        self._compare_results(qs_original, qs_hybrid,
                             "Original VMP", "Hybrid", spec_name)
        self._compare_results(qs_original, qs_clustered_hybrid,
                             "Original VMP", "Clustered Hybrid", spec_name)
        self._compare_results(qs_original, qs_hybrid_block,
                             "Original VMP", "Hybrid Block", spec_name)
        self._compare_results(qs_original, qs_clustered_hybrid_block,
                             "Original VMP", "Clustered Hybrid Block", spec_name)
        self._compare_results(qs_original, qs_end2end,
                             "Original VMP", "End2End Padded", spec_name)
        self._compare_results(qs_original, qs_clustered_end2end,
                             "Original VMP", "Clustered End2End", spec_name)
