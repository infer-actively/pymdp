...
class Agent(Module):
    ...
    def __init__(
        ...
        if self.use_inductive and self.H is not None:
            ...
            self.I = jtu.tree_map(
                lambda x: jnp.expand_dims(jnp.zeros_like(x), 1), self.D
            )
        ...
        if E is None:
            self.E = jnp.ones((self.batch_size, len(self.policies))) / len(
                self.policies
            )
        else:
            self.E = E
    ...
    def _construct_I(self):
        return control.generate_I_matrix(
            self.H, self.B, self.inductive_threshold, self.inductive_depth
        )
    ...
    def learning(
        ...
        **kwargs,
        ...
        agent = self
        if self.learn_A:
            o_vec_seq = jtu.tree_map(
                lambda o, dim: nn.one_hot(o, dim), outcomes, self.num_obs
            )
            qA = learning.update_obs_likelihood_dirichlet(
                self.pA, o_vec_seq, beliefs_A, self.A_dependencies, lr=lr_pA
            )
            E_qA = jtu.tree_map(
                lambda x: maths.dirichlet_expected_value(x), qA
            )
        ...
        if self.learn_B:
            ...
            actions_onehot = jtu.tree_map(
                lambda a, dim: nn.one_hot(a, dim, axis=-1),
                actions_seq,
                self.num_controls,
            )
            ...
            E_qB = jtu.tree_map(
                lambda x: maths.dirichlet_expected_value(x), qB
            )
